...
output_path_P = "hdfs://namenode:9000/data/results/q3_output/rdd_tuple/percentile"
output_path_MM = "hdfs://namenode:9000/data/results/q3_output/rdd_tuple/MinMax"
...
def rdd_filter(tupla):
    ...
    return carrier in carriers and cancelled == 0.0

# === FUNZIONI PER L'ELABORAZIONE RDD ===
def convert_to_tuple(tupla):
    carrier         = tupla[idx_carrier]
    crs_dep_time    = tupla[idx_time]
    dep_delay       = tupla[idx_delay]

    # Calcolo dell'ora
    hour = int(crs_dep_time) // 100
    delay = float(dep_delay) if dep_delay is not None else 0.0
...
if __name__ == "__main__":
    ...
    idx_time            = columns.index("CRS_DEP_TIME")
    ...
    for i in range(0, 10):
        ...
        df_raw = spark.read.option("basePath", input_path).parquet(*paths_to_read).rdd.map(tuple)

        # Filtriamo mantenendo perÃ² l'intera ROW (struttura intatta)
        # Convertiamo CANCELLED a float per evitare errori in caso sia memorizzato come stringa o decimale
        rdd_filtered = df_raw.filter(rdd_filter)
        rdd_filtered.cache()
